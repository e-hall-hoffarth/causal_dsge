...
parameters alppha       ${\alpha}$      (long_name='capital share')
    ...
    sig                 ${\hat{\sigma}}$ (long_name='shock standard deviation')
    ;
...
shocks;
    @#if money_growth_rule==0
        var eps_nu; stderr sig;
    @#else
        var eps_m; stderr sig;
    @#endif

var eps_z; stderr sig;
var eps_a; stderr sig;
end;
