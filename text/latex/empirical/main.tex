...
\abstract{DAGs are a type of graphical model that can under some condtions allow for causal inferences to be made from observational data. This paper argues that these conditions, while nuanced, are likely reasonable in many economic contexts. Despite the clear value of such a technique in economic applications, little empirical work has been done in this area. Therefore, this paper aims to address this by considering the application of DAGs to simulated and real macroeconomic timeseries data. The results indicate that DAGs perform reasonably well at identifying the expected causal pathways in simulated and real data when variables are not subject to selection and are small in number relative to the sample size, but may struggle otherwise.}
